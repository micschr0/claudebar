...
- [Architecture overview](overview.md) - Layered module graph and invariants of the claudebar binary: model → render → segment → themes/styles, the single-render-path invariant, render-composition sub-layers (writer/bar/width/float), auto-layout, and tui feature gating.
