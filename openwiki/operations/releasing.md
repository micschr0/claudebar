...
`[workspace.metadata.dist]` in `Cargo.toml` is the config cargo-dist generates
from: the **four targets** listed explicitly (musl is not in dist's default
suite), `auditable = true` (embeds dependency metadata for `cargo audit` bin
scanning), `checksum = "sha256"`, `installers = ["homebrew"]` (the only
cargo-dist-built installer — `install.sh` is shipped independently from the
repo root), `publish-prereleases = true` (so beta tags drive the Homebrew job
into the `@beta` branch instead of short-circuiting it), the pinned
`cargo-dist-version = "0.32.0"`, and `allow-dirty = ["ci"]`, which accepts that
the generated workflow is hand-edited (actions pinned to commit SHAs for zizmor
compliance) and tells `dist generate --check` not to treat that divergence as
drift.
